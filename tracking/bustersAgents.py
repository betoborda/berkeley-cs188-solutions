    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        minDistanceAndAction = (float("inf"), None)

        for ghostDist in livingGhostPositionDistributions:
            ghostPos = ghostDist.argMax()

            minDistanceAndAction = min([minDistanceAndAction] + \
                [(self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, newAction), ghostPos), newAction) \
                    for newAction in legal])

        return minDistanceAndAction[1]
