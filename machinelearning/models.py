...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        a1 = nn.ReLU(z1)
        return nn.AddBias(nn.Linear(a1, self.w2), self.b2) # z2
...
class DigitClassificationModel(object):
    ...
    def run(self, x):
        ...
        a2 = nn.ReLU(z2)
        return nn.AddBias(nn.Linear(a2, self.w3), self.b3) # z3
    ...
    def train(self, dataset):
        ...
        """
        "*** YOUR CODE HERE ***"
        while dataset.get_validation_accuracy() < self.minAccuracy:
            for x, y in dataset.iterate_once(self.batchSize):
                grad_w1, grad_b1, grad_w2, grad_b2, grad_w3, grad_b3 = \
                    nn.gradients(self.get_loss(x, y), [self.w1, self.b1, self.w2, self.b2, self.w3, self.b3])
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        self.w1h = nn.Parameter(300, 300)
        self.b1  = nn.Parameter(1, 300)
        self.b1h = nn.Parameter(1, 300)
        
        self.w2 = nn.Parameter(300, 100)
        self.b2 = nn.Parameter(1, 100)
        
        self.w3 = nn.Parameter(100, len(self.languages))
        self.b3 = nn.Parameter(1, len(self.languages))

        self.batchSize    = 100
        self.learningRate = -0.05
        self.minAccuracy  = 0.85
    ...
    def run(self, xs):
        ...
        for x in xs[1:]:
            h = nn.Add(
                nn.AddBias(nn.Linear(x, self.w1), self.b1), 
                nn.AddBias(nn.Linear(h, self.w1h), self.b1h))
    ...
    def train(self, dataset):
        ...
        while dataset.get_validation_accuracy() < self.minAccuracy:
            for x, y in dataset.iterate_once(self.batchSize):
                grad_w1, grad_b1, grad_w1h, grad_b1h, grad_w2, grad_b2, grad_w3, grad_b3 = \
                    nn.gradients(self.get_loss(x, y), [self.w1, self.b1, self.w1h, self.b1h, self.w2, self.b2, self.w3, self.b3])

                self.w1.update (grad_w1,  self.learningRate)
                self.b1.update (grad_b1,  self.learningRate)
                self.w1h.update(grad_w1h, self.learningRate)
                self.b1h.update(grad_b1h, self.learningRate)
                self.w2.update (grad_w2,  self.learningRate)
                self.b2.update (grad_b2,  self.learningRate)
                self.w3.update (grad_w3,  self.learningRate)
                self.b3.update (grad_b3,  self.learningRate)
